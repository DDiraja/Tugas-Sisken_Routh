...
def input_Polynomial(polynomial):
   deg=int(input('Jumlah Orde: '))
...
def get_Routh_Array(RouthArray):
...
def RouthHurwitz(polynomial):
    ...
    row1 = []
    expr = get_Expr(f's ** {degree}')
    row1.append(expr)

    for i in range(degree, -1, -2):
        expr = get_Expr(polynomial[i])
        row1.append(expr)

    row2 = []
    expr = get_Expr(f's ** {degree - 1}')
    row2.append(expr)
...
print("Tabel Routh : ")
get_Routh_Array(RouthHurwitz(polynomial))
valueK=input('nilai K : ')
K_value(valueK,RouthHurwitz(polynomial))